...
export class SupportTicketChatComponent implements OnChanges {
  ...
  constructor(private route: ActivatedRoute, private supportTicketService: SupportTicketService, private toastr: ToastrServiceService, private router: Router, private sessionStorage: SessionStorageService, private formBuilder: FormBuilder) {
  ...
  loadData() {
    ...
    this.supportTicketService.GetSupportTicketById(input).subscribe(res => {
      if (res.status) {
        this.chats = res.data;
        this.resetForm();
        var ticket = this.chats.find(m => m.supportTicketId == this.ticketID);
        this.ticketStatus = this.supportStatus.find(m => m.id == ticket.ticketStatus).name;
        this.isCloseOrCancel = ticket.ticketStatus == 3 || ticket.ticketStatus == 4 || ticket.ticketStatus == 2;
        setTimeout(function () {
  ...
  getDateFormat(date, format) {
    ...
    if (today == nowDate)
      return 'Today';
    else if (yesterday == nowDate)
      return 'Yesterday';
    else if (tommorrow == nowDate)
      return 'Tommorrow';
    else
      return nowDate;
  ...
  addChat() {
    if (this.SupportTicketsForm.valid) {
      this.supportTicketService.AddSupportTicket(this.SupportTicketsForm.getRawValue()).subscribe(res => {
        if (res.status) {
          this.loadData();
        }
        else {
  ...
  resetForm() {
    ...
    this.SupportTicketsForm.get("applicationId").setValue(this.chats.find(m => m.supportTicketId == this.ticketID).applicationId);
  }
  backToTicket() {
    this.back.next();
  }
  changeStatus(statusId) {
    ...
    this.supportTicketService.UpdateSupportTicketStatus(input).subscribe(res => {
      if (res.status) {
