...
@Component({
  ...
  styleUrls: ['./student-application.component.sass']
})
export class StudentApplicationComponent implements OnInit {
  modalTitle = "Add Student Application";
  faAngleUp = faAngleUp;
  ...
  loadStudentApplication() {
    ...
    // let getSatgeCount = this.courseService.getStageCount(1);
    // alert(getSatgeCount);
    $('#loader').show();
    let input = {
      size: 10,
    ...
    this.dtOptions = {
      ...
      columns: [{ name: '1', orderable: false }, { name: 'avtar', orderable: false }, { name: 'Name', orderable: false }, { name: 'Country', orderable: false },
      { name: 'Status', orderable: false }, { name: 'DueDate', orderable: false }, { name: 'progress', orderable: false }, { name: 'action', orderable: false }],
      autoWidth: false
    }
  ...
  openApplication(content: any, id: any, mode: any = 'edit') {
    ...
    if (id <= 0) {
      this.modalTitle = "Add Student Application";
      this.isView = false;
  ...
  SaveApplication() {
    ...
    if (this.Applicationform.valid) {
      ...
      this.applicationService.saveApplication(input).subscribe(res => {
        ...
        if (res.status) {
          this.toastr.SuccessToastr("Student application is save successfully.");
          this.modalService.dismissAll();
          $(".table").DataTable().ajax.reload();

        }
        else {
          this.toastr.ErrorToastr("Student application is not saved.");
        }
        $('#loader').hide();
      }, (err: any) => {
        $('#loader').hide();
